...
public class ViewGraphic extends TextureView {
    ...
    private boolean graphicInitialised, cpuTotalD, cpuAMD, memUsedD, memAvailableD, memFreeD,
			cachedD, thresholdD;
    private int processesMode, graphicMode, yTop, yBottom, xLeft, xRight, yBottomTextSpace = 25,
			xLeftTextSpace = 10, yLegendSpace = 8, graphicHeight, graphicWidth, minutes, seconds,
			intervalTotalNumber, memTotal, thickParam, thickGrid, thickEdges, tempVar, textSize,
			textSizeLegend, yTopSeparation;
    private String readIntervalText, updateIntervalText, graphicIntervaWidthlText, recordingText
			= "Recording";
    //	private Path graphicPath;
    private Rect bgRect;
    private Paint bgPaint, circlePaint, textPaintRecording, textPaintLegend, textPaintLegendV,
			textPaintInside, linesEdgePaint, linesGridPaint, cpuTotalPaint, cpuAMPaint,
...
//    private List<Float> cpuTotal, cpuAM;
    ...
    protected void onDrawCustomised(Canvas canvas, Thread thread) {
        ...
        // 		return;
        // is used before every time canvas.drawX() is called to avoid a fatal bug present on
		// Android 4.0-5.0:
        // http://stackoverflow.com/questions/20694067/textureview-throwing-fatal-signal-11-after
...
//                    if (paint == null) {
//                        paint = getPaint((Integer) l.get(n).get(C.pColour), Paint.Align.CENTER,
//								12, false, thickParam);
//                        paints.put((String) l.get(n).get(C.pId), paint);
//                    }
//                    if (processesMode == C.processesModeShowCPU) // processesMode==0 CPU usage,
//                    	// processesMode==1 Memory
//                        drawLineFloat((List<Float>) l.get(n).get(C.pFinalValue), canvas, paint);
//                    else
//                        drawLineInteger((List<Integer>) l.get(n).get(C.pTPD), canvas, paint);
//                }
//            }
        ...
        //		if (mThread.isInterrupted())
        //			return;
        //		canvas.drawText("80%", tempVar, yTop + graphicHeight*0.2f + yLegendSpace,
		//		textPaintLegendV);
        if (mThread.isInterrupted())
            return;
        ...
        //		if (mThread.isInterrupted())
        //			return;
        //		canvas.drawText("60%", tempVar, yTop + graphicHeight*0.4f + yLegendSpace,
		//		textPaintLegendV);
        if (mThread.isInterrupted())
            return;
        ...
        //		if (mThread.isInterrupted())
        //			return;
        //		canvas.drawText("40%", tempVar, yTop + graphicHeight*0.6f + yLegendSpace,
		//		textPaintLegendV);
        if (mThread.isInterrupted())
            return;
        ...
        //		if (mThread.isInterrupted())
        //			return;
        //		canvas.drawText("20%", tempVar, yTop + graphicHeight*0.8f + yLegendSpace,
		//		textPaintLegendV);
        if (mThread.isInterrupted())
            return;
        ...
        if (mThread.isInterrupted())
            return;
        canvas.drawText(graphicIntervaWidthlText + " " + mSR.getIntervalWidth() + " dp",
				xLeft + 15, yTop + 5 + yTopSeparation * 3, textPaintInside);

        // Recording text
//        if (mSR.isRecording()) {
//            if (mThread.isInterrupted())
//                return;
//            canvas.drawText(recordingText, xRight - 40, yTop + 5 + yTopSeparation,
//					textPaintRecording);
//            if (mThread.isInterrupted())
//                return;
...
//        }
    ...
    private void initializeGraphic() {
        ...
        cpuTotalPaint = getPaint(res.getColor(R.color.process1), Paint.Align.CENTER, 12, false,
				thickParam);
        cpuAMPaint = getPaint(res.getColor(R.color.process2), Paint.Align.CENTER, 12, false,
				thickParam);
    ...
    void setParameters(boolean cpuTotalD, boolean cpuAMD, boolean memUsedD, boolean memAvailableD, boolean memFreeD, boolean cachedD, boolean thresholdD) {
